 ...
 *	entonces, dado que $S$ es la bola cerrada de centro $num$ y radio $2t$, sabemos que $S$ es no vac\'io debido a que $\mathbb{Z}$ es no vac\'io y esto nos lleva a concluir que $num$ siempre estar\'a definido, por lo cual, para $t = 0$, $S = \{num\}$. Con lo que queda demostrado el teorema. Sin embargo, se pueden extraer m\'as cosas de este resultado, tales como
